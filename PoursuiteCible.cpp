...
int main(int argc, char *argv[]) {
    int nombre_de_points, n, increment, m ,t;
    ...
    CImg<unsigned char>::iterator Valeur_area;
    CImg<double>::iterator pointeur; // pointeur de recuperation des valeurs de niveau de gris de l'image
    CImg<unsigned char> Detail, areaOfResearch; // recuperation du motif en poursuite
    ...
    double *Valeur_motif, *lastValeur_motif, *Derivee_temporelle;
    int *Point_x, *Point_y;
    ...
    while (ok) {
        ...
        if (increment == 1) {
            ...
            nombre_de_points = Max(nombre_de_points, 0);
            ...
            lastValeur_motif = ALLOCATION(dim, double);
...
//            while (!ok) {
//                if (carre_x[0] - u_amplitude > 0 && carre_x[1] + u_amplitude < nx
//                    && carre_y[0] - v_amplitude > 0 && carre_y[1] + v_amplitude) {
//                    areaOfResearch.crop(carre_x[0] - u_amplitude, carre_y[0] - v_amplitude, carre_x[1] + u_amplitude,
//                                        carre_y[1] + v_amplitude);
//                    ok = true;
//                } else {
//                    u_amplitude--;
//                    v_amplitude--;
//                }
//            }
//            Valeur_area = areaOfResearch.begin();
...
//            int offsetX = 0;
//            int offsetY = 0;
//            for (int v = 0; v < v_amplitude*2 + carre_y[1]-carre_y[0]; ++v) {
//                for (int u = 0; u < u_amplitude*2 +carre_x[1]-carre_x[0]; ++u) {
//                    val = 0.0;
//                    for (int y = 0; y < ny; ++y) {
//                        for (int x = 0; x < nx; ++x) {
//                            val += abs((float) (Valeur_image[(y + offsetY) * ny + (x + offsetX)] -
//                                                Valeur_motif[y * ny + x])) /(float)(dim);
//                        }
//                    }
//                    if (val < SAD) {
//                        SAD = val;
...
//            delta_y += SAD;
        ...
        memcpy(Valeur_motif, lastValeur_motif, dim);
    ...
    DESALLOCATION(lastValeur_motif);
