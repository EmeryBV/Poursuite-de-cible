...
if __name__ == '__main__':
    ...
    # pointX_start, pointY_start = 220, 240
    # pointX_end, pointY_end = 240, 260

    # pointX_start, pointY_start = 410, 140
    # pointX_end, pointY_end = 430, 160

    pointX_start, pointY_start = 440, 240
    pointX_end, pointY_end = 460, 260
    ...
    for i in range(2, numberOfFile - 1):
        ...
        for offsetY in range(-v_amplitude, v_amplitude + 1):
            for offsetX in range(-u_amplitude, u_amplitude + 1):
                ...
                # img2 = cv2.imread(filename)
                # start_point_Area = (
                #     searchX_start + offsetX + u_amplitude - 1, searchY_start + offsetY + v_amplitude - 1)
                # end_point_Area = (searchX_start + offsetX + u_amplitude + lenghtX + 1,
                #                   searchY_start + offsetY + v_amplitude + lenghtY + 1)
                ...
                for v in range(0, lenghtY + 1):
                    for u in range(0, lenghtX + 1):
                        ...
                        val = val + abs(((img2[pointY_start + v, pointX_start + u] - moyenneImg1)[0]
                                        - (img[searchY_start + v + offsetY + v_amplitude, searchX_start +
                                               u + offsetX + u_amplitude] - moyenneImg2)[0]) ** 2)/math.sqrt(sigmaImg1*sigmaImg2)
                        # print(val)
    ...
    out = cv2.VideoWriter('Poursuite de cible avec motif mouvant.avi', cv2.VideoWriter_fourcc(*'DIVX'), 15, size)
